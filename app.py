...
with gr.Blocks(title=CONFIG["app"]["title"]) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            events_table = gr.Dataframe(
                headers=["事件编号", "事件类型", "风险等级", "开始秒", "结束秒", "持续时长（秒）", "告警文本"],
                label="异常事件详情",
            )
    with gr.Row():
        clips = gr.File(label="异常视频片段", file_count="multiple")
        thumbnails = gr.Gallery(label="缩略图", columns=4, height=260)
    benchmark = gr.Dataframe(headers=["模型方案", "是否可用", "平均延迟（毫秒）", "吞吐率（帧/秒）", "显存峰值（MB）", "错误说明"], label="FP32 与 FP16 性能对比")
    ...
    run_button.click(
        ...
        outputs=[original_video, video_info, hardware_info, events_table, clips, thumbnails, benchmark, result_file, result_json],
    )
